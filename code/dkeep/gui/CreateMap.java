...
public class CreateMap extends JPanel implements MouseListener{
	...
	private char[][] full_map;
	private char[][] no_chrs_map;
	...
	CreateMap(int height, int width){
		key_b=false; door_b=false; hero_b=false;
		ogres = new ArrayList<GameCharacter>();
		this.addMouseListener(this);
		CreateMap.height=height; CreateMap.width=width;
		ogres_number=0;
		full_map = new char[height][width];
		no_chrs_map = new char[height][width];

		for (int linha = 0; linha < height; linha++)
			for (int coluna = 0; coluna < width; coluna++) 
				no_chrs_map[linha][coluna] = (full_map[linha][coluna] = (linha == 0 || coluna == 0 || 
	...
	public void paint(Graphics g){
		...
		for (int i = 0; i < height; i++) {
			for(int j=0; j< width;j++){
				g.drawImage( this.char_to_img.get( full_map[i][j]) , x , y , 50 , 50 , null);
	...
	private void insertWall( int x , int y){
		if(full_map[x][y]!='X'){
			full_map[x][y]= (no_chrs_map[x][y]='X');
		}
		else{
			full_map[x][y]= (no_chrs_map[x][y]=' ' );
		}
		repaint();
	}
	private void insertExit( int x , int y){
		if(full_map[x][y]!='I'){
			if(contaisComp('I'))
				removesComp('I');
			full_map[x][y] = (no_chrs_map[x][y]='I');
			door_b=true;
	...
	private void insertKey( int x , int y){
		if(full_map[x][y]!='k'){
			if(contaisComp('k'))
				removesComp('k');
			full_map[x][y] = (no_chrs_map[x][y] = 'k');
			key_b=true;
